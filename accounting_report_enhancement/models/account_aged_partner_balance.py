                    ...
                    for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
                        ...
                        for partner_id, aml_data_list in aml_data_by_partner.items():
                            partner_values = {
                                ...
                                'partner_name': None,
                                'move_date': None,
                                'move_invoice_user_id': None,
                                'move_invoice_date_due': None,
                                'move_invoice_custom_date_due': None,
                            }
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0,
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                partner_id = self.env['res.partner'].browse(query_res.get('partner_id')[0]) if len(
                    query_res['partner_id']) == 1 else None
                move_id = self.env['account.move'].browse(query_res.get('move_id')[0]) if len(
                    query_res['move_id']) == 1 else None
                invoice_date_due = query_res['due_date'][0] if len(query_res['due_date']) == 1 else None
                days_overdue = (date_to - invoice_date_due).days if date_to > invoice_date_due else 0
                custom_date_due = days_overdue
                if custom_date_due == 0:
                    custom_date_due = (date_to - invoice_date_due).days
                rslt.update({
                    ...
                    'move_id': query_res['move_id'][0] if query_res['move_id'] else None,
                    'partner_name': partner_id.name if partner_id else None,
                    'move_date': move_id.invoice_date if move_id else None,
                    'move_invoice_user_id': move_id.invoice_user_id.name if move_id and move_id.invoice_user_id else None,
                    'move_invoice_date_due': days_overdue if move_id else None,
                    'move_invoice_custom_date_due': custom_date_due if move_id else None,
                })
            else:
                rslt.update({
                    'due_date': None,
