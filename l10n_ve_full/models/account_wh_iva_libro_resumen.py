...
class resumen_libros(models.TransientModel):
    ...
    line = fields.Many2many(comodel_name='account.wh.iva.libro.pdf.resu', string='Lineas')
    ...
    def get_invoice(self, accion):
        t = self.env['account.wh.iva.libro.pdf.resu']
        d = t.search([])
        # d.unlink()
        if accion == "factura":
            cursor_resumen = self.env['account.move.line.resumen'].search([
    ...
    def generate_xls_report(self):
        self.env['account.wh.iva.libro.pdf.resu'].search([]).unlink()
        # self.get_invoice()
