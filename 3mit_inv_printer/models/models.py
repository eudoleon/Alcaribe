...
class AccountMove(models.Model):
    ...
    def printFactura(self):
        ...
        # ✅ Nueva validación directa: Si monto existe, debe ser <= amount_total
        if self.monto:
            if round(self.monto, 2) > round(self.amount_total, 2):
                raise UserError("El monto de pago no puede ser mayor al monto total de la factura.")
        
        # ✅ Opcional: Validar que no coloquen monto <= 0
        if self.monto and self.monto <= 0:
            raise UserError("El monto de pago debe ser mayor que cero.")
